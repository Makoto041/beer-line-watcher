...
const SOURCE_ID = "alwayslovebeer-calendar";
const CALENDAR_ID = "84g71qbda1g27dr7i1arerq454@group.calendar.google.com";
 ...
 * Supports formats: YYYYMMDD, YYYYMMDDTHHmmss, and YYYYMMDDTHHmmssZ
 */
function parseICalDate(dateStr: string): Date | null {
  ...
  // Format: YYYYMMDDTHHmmss or YYYYMMDDTHHmmssZ (ISO-like with optional time and Z suffix)
 ...
 * Supports various formats: <a href="URL">, http://..., https://...
 */
function extractUrlFromDescription(description: string): string | null {
  // Try to extract URL from <a href="...">
  const anchorMatch = description.match(/<a\s+href="([^"]+)"/i);
  if (anchorMatch && anchorMatch[1]) {
    return anchorMatch[1];
  }

  // Try to extract plain URL (first one found)
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (trimmed.startsWith("SUMMARY:")) {
          ...
          // Handle DTSTART:20240101, DTSTART;VALUE=DATE:20240101, and DTSTART;TZID=...:...
      ...
      // If no URL found, create a fallback URL to Always Love Beer events page
      if (!url) {
 ...
 */
export async function crawlAlwaysLoveBeerCalendar(): Promise<CrawledItem[]> {
  try {
    ...
    console.log(`Parsed ${items.length} events from Always Love Beer Calendar`);
    return items;
  } catch (error) {
    console.error("Error crawling Always Love Beer calendar:", error);
    return [];
  }
}
